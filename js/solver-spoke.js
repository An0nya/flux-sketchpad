 ...
 * partition, per-zone curvature).  Facet budget + reflectivity come from input.limits; facetType and
 * minDistance default from the Paint controls (see Solvers.settingsOf). */
(function (root) {
  'use strict';
  const RF = root.RF;
  RF.Solvers.register({
    id: 'spoke', name: 'Spoke (default)', version: '1.2', modes: ['paint'],
    settings: [
    ...
    solve(input, s) {
      const scene = { source: input.source, target: input.target, envelope: input.envelope,
        modeA: { paint: input.paint.cells, budget: input.limits.maxFacets, facetType: s.facetType, reflectivity: input.limits.reflectivity, minDistance: s.minDistance } };
